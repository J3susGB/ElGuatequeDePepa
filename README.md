...
#### Se trata del proyecto de una página web que me encargó una empresa de animación de eventos, que montan una cabina hinchable, y dentro un fotomatón/videomatón.
  ...
  - *Estructura principal* --> Se trata de una onePage, estructurada en cinco bloques o partes, con una página extra de apoyo donde se regisgra toda la política de seguridad y el aviso legal de la empresa, a la que se puede hacer a través del botón -más información- del banner emergente de las cookies o pulsando los enlaces habilitados para ellos en el footer de la onePage
  ...
  - *Bloque ¿Cómo funciona?* --> Descripción a fondo de los servicios que se ofrecen y como se lleva a cabo el evento. Finaliza con una galería foto hecha carrusel, donde aparecen distintas imágenes representativas de eventos que la empresa ha realizado
...
#### La página está construida con HTML5, CSS3, PHP y JavaScript. Se trata de una página web responsive, hecha cuidadosamente teniendo en cuenta todas las franjas de tamaño de pantalla. Respetando el diseño, la página se adapta al ancho de la pantalla en la que se visualiza, lo que mejora considerablemente la experiencia visual de los usuarios. 

#### Paso a explicar las partes que tienen una funcionalidad específica:

- *Globales* --> De forma general, la web tiene un script que controla la velocidad de desplazamiento al hacer scroll, y dos botones fijados a la parte inferior y derecha de la web, que acompañan a usuario en todo momento al hacer scroll, uno es botón que te lleva directamente al chat con la empresa en la aplicación de whatsApp, al que le he implementado un evento de javascript de tipo click para su uso, y el otro, es una flecha que al pulsarla, te lleva directamente al inicio de la página.
...
- *Header* --> Lo divido en dos partes prinpipales, usando grid con CSS: header__top y header__main. 
  ...
  Los script de ocultacion del logo y espacio y desplazamiento del menú de navegación, a la inversa hace lo contrario, es decir, si el usuario vuelve al principio de la página, el logo vuelve a aparecer y el menú se desplaza a su lugar original.
